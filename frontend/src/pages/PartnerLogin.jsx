...
function PartnerLogin() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const response = await api.post('/auth/food/login', { email, password });
      toast.success(response.data.message || 'Login successful!');
      localStorage.setItem('partner', JSON.stringify(response.data.food));
      localStorage.setItem('role', 'partner');
      navigate('/partner/dashboard');
    } catch (err) {
      toast.error(err.response?.data?.message || 'Invalid partner email or password');
    } finally {
      setLoading(false);
    }
  };
    ...
    <main className="flex min-h-screen items-center justify-center bg-surface relative overflow-hidden">
      ...
      <div className="relative w-full max-w-md bg-white rounded-xl shadow-xl overflow-hidden z-10 p-6 flex flex-col transition-all duration-300 hover:shadow-2xl">
        ...
        <div className="flex flex-col items-center mb-6">
          ...
          <h1 className="text-3xl font-bold text-on-surface mb-1 tracking-tight text-center">Welcome Back</h1>
          <p className="text-sm text-secondary text-center font-semibold">Partner Portal Access</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4 w-full">
          ...
          <div className="flex flex-col gap-1">
            <div className="flex justify-between items-center ml-1 mr-1">
              <label className="text-xs font-semibold text-secondary uppercase tracking-wider" htmlFor="password">
                Password
              </label>
              <button className="text-xs font-semibold text-slate-500 hover:text-primary transition-colors" type="button">
                Forgot?
              </button>
            </div>
            <div className="relative group">
        ...
        <div className="mt-6 text-center flex flex-col gap-2">
          ...
          <p className="text-xs text-secondary">
            ...
            <Link to="/partner/register" className="text-primary font-bold hover:underline">
